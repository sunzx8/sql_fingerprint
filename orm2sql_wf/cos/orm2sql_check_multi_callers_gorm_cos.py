...
CODE_ORM_MYSQL_SQL_EXTRACT = \
    ...
    "- 软删除：带有DeletedAt字段的模型会自动添加WHERE deleted_at IS NULL条件\n" \
...
CODE_ORM_MYSQL_SQL_FORMAT = \
    ...
    "   - 结构体字段名的蛇形命名\n" \
    "4. 对于带有DeletedAt字段的模型，SELECT查询会自动添加WHERE deleted_at IS NULL条件\n" \
    "5. 对于Delete操作，需要注意：\n" \
    "   - 当使用`.Where().Delete(structInstance)`时，GORM会同时应用手动WHERE条件和结构体主键条件\n" \
...
if __name__ == '__main__':
    ...
    output_file = '/data/local_disk0/shawn/api_benchmark/cos/cos_results_multi_callers_529_1316.json'
